...
from inference_api import is_anomaly_hub, load_hub_model
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Continuously infer anomalies from app-level window CSVs.")
    parser.add_argument("--windows-dir", required=True)
    parser.add_argument("--hub-model-dir", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    hub_model_dir = Path(args.hub_model_dir)
    ...
    if not windows_dir.is_dir():
        ...
        return 1
    if not hub_model_dir.is_dir():
        print(f"hub model directory not found: {hub_model_dir}", file=sys.stderr)
        return 1

    model, scaler, threshold = load_hub_model(str(hub_model_dir))
    ...
    while True:
        ...
        for start_ms, end_ms, path in iter_window_files(windows_dir):
            ...
            try:
                prediction = is_anomaly_hub(model, scaler, threshold, str(path))
